...
const MSGS: &[(Msg, &str, &str)] = &[
    ...
    (
        Msg::ExternalPackHint,
        "通过『加载芯片描述文件』或『从 CMSIS 包生成』导入的芯片，在此处选择型号",
        "Chips imported via 'Load Chip File' or 'Generate from CMSIS Pack' are selected here",
    ),
    (Msg::Brand, "品牌", "Brand"),
    ...
    (
        Msg::ArmIndexFailed,
        "拉取 ARM 在线索引失败（请检查网络）: {}",
        "Failed to fetch the ARM online index (check your network): {}",
    ),
    (
    ...
    (
        Msg::ArmSearchHint,
        "如 GD32 / STM32F4，留空列出全部",
        "e.g. GD32 / STM32F4; empty lists all",
    ),
    (
        Msg::ArmSearching,
        "正在搜索 ARM 索引（关键字: {}）...",
        "Searching the ARM index (keyword: {})...",
    ),
    (Msg::ArmSearchingTitle, "搜索中...", "Searching..."),
    (
        Msg::ArmDownloading,
        "正在从 ARM 索引下载并生成: {}（可能需要较长时间）...",
    ...
    (
        ...
        "添加到外部芯片包",
        "Add to External Packs",
    ),
    (
        Msg::ArmNoResult,
        "无匹配结果，请输入关键字（如 GD32 / STM32F4）后搜索",
        "No matches. Enter a keyword (e.g. GD32 / STM32F4) and search",
    ),
    // ---- 固件烧录 ----
