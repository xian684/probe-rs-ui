...
const MSGS: &[(Msg, &str, &str)] = &[
    ...
    // ---- Target 生成器 ----
    (Msg::TargetGenerator, "Target 生成器", "Target Generator"),
    ...
    (Msg::TgBrowseFile, "浏览文件...", "Browse File..."),
    (Msg::TgBrowseDir, "浏览目录...", "Browse Directory..."),
    (Msg::TgOutputDir, "输出目录:", "Output directory:"),
    (
        Msg::TgOutputDirHint,
        "生成的 <family>.yaml 将写入此目录",
        "Generated <family>.yaml files will be written here",
    ),
    (Msg::TgOnlySupported, "仅生成 probe-rs 已支持的芯片族", "Only supported families"),
    (
    ...
    (Msg::TgGenerate, "生成 Target 定义", "Generate Target"),
    (
        Msg::TgGenerating,
        "正在从 CMSIS Pack 生成 target 定义，请稍候...",
        "Generating target definitions from the CMSIS pack...",
    ),
    (Msg::TgResult, "生成结果", "Generated Results"),
    (
        Msg::TgNoResult,
        "尚未生成。请选择输入后点击『生成 Target 定义』",
        "Nothing generated yet. Pick an input and click 'Generate Target'",
    ),
    (Msg::TgFamily, "芯片族", "Family"),
    (Msg::TgVariants, "型号数", "Variants"),
    (Msg::TgFlashAlgos, "Flash 算法", "Flash algos"),
    (Msg::TgOutputFile, "输出文件", "Output file"),
    (
        Msg::TgInputMissing,
        "输入路径不存在: {}",
    ...
    (
        Msg::TgOutputDirMissing,
        "请先选择输出目录",
        "Please pick an output directory first",
    ),
    (
