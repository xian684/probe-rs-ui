...
- 🌐 **ARM 在线索引**：高级芯片配置中可搜索 [Keil.pidx](https://www.keil.com/pack/index.pidx) 公共索引（按关键字如 `GD32` / `STM32F4` 过滤），选中 Pack 后一键下载并生成芯片描述，自动导入外部芯片包视图。
...
```
probe-rs-ui/
├── build.rs         Windows 资源脚本（版本信息）
└── src/
    ├── main.rs      程序入口：窗口配置、字体加载与 eframe 启动
    ├── app/         应用状态中枢：状态/入口、配置持久化、事件分发、烧录与内存操作
    ├── panels/      界面面板：顶栏、设备检测（手动选型/高级芯片配置）、中央面板（烧录/内存/RTT/ARM 索引）等
    ├── worker/      后台线程：命令分发、探针连接、烧录、内存读写、CMSIS Pack 集成、ARM 在线索引
    ├── chips.rs     内置芯片库与品牌分组
    ├── config.rs    config.toml 持久化
    ├── firmware.rs  固件扫描与格式识别
    ├── rtt.rs       RTT 会话与通道读写
    ├── fonts.rs     CJK 字体加载
